...
| `--process_bike-tags` | Not enabled | If included, some normally excluded osm tags (specific versions of `highway="track"` and `highway="path"`) are modified to `highway="bike_suitable"` and treated as cycleways which enriches the cycling network. |

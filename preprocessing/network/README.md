...
|---|---|---|
| `--target-crs` | `"EPSG:2056"` | Desired output CRS for MATSim-ready files. Use a projected CRS in metres that is appropriate for the study area. |
| `--sample-day` | `"dayWithMostTrips"` | Service day used by [`Gtfs2TransitSchedule`](https://github.com/matsim-org/pt2matsim/wiki/Creating-an-unmapped-MATSim-transit-schedule). Can also be set to a specific date, for example `"20260422"`. |
| `--additional-line-info` | `"schedule"` | Additional line information argument passed to `Gtfs2TransitSchedule`. This can be useful for retaining extra route or line information during schedule creation. |
| `--shape-buffer-degrees` | `0.0` | Buffer applied to the project shapefile after it is reprojected to WGS84. This should normally stay at `0.0`, but a small positive value can be used if stops or OSM features very close to the boundary are accidentally excluded. |
| `--overwrite` | Not enabled | If included, existing output folders for the scenario are deleted before the pipeline is rerun. Use this when repeating a run after changing input files, config templates, or pipeline settings. |
| `--process-bike-tags` | Not enabled | If included, some normally excluded osm tags (specific versions of `highway="track"` and `highway="path"`) are modified to `highway="bike_suitable"` and treated as cycleways which enriches the cycling network. |
| `--prostprocess-bike-net` | Not enabled | If included, removes `bike` as allowed mode from all links above a speed threshold of 50km/h, with the exception of any links with cycle/mtb routes on them, useful when high-class roads are sometimes used by bicycles. |
| `--java-runner` | `"mvn"` | Command used to call Maven. On some Windows setups, `"mvn.cmd"` may be required instead. |
| `--osm-config-template` | `configs/pt2matsim/osm2multimodal_template.xml` | Path to the PT2MATSim `Osm2MultimodalNetwork` config template. The pipeline overwrites only the relevant input/output paths and CRS; the modelling settings remain defined in this template. |
| `--mapper-config-template` | `configs/pt2matsim/transit_mapper_template.xml` | Path to the PT2MATSim public transit mapper config template. The pipeline overwrites only the relevant input/output paths and CRS; the mapping settings remain defined in this template. |
| `--osm-override` | Not set | Override a parameter in the OSM converter config from the command line. Can be used multiple times. Example: `--osm-override maxLinkLength=500.0`. |
| `--mapper-override` | Not set | Override a parameter in the public transit mapper config from the command line. Can be used multiple times. Example: `--mapper-override maxLinkCandidateDistance=90.0`. |
